...
<tr>
<th >Create Picking Slip Deliveries</th>
...
<table class="table table-striped" >
  ...
  if (checkBox.checked == true){
    ...
    <th   class="highlights"><a class="btn btn-dark" id="myLink" href="{{ route('pickingslip.index') }}">Picking</a></th>
  ...
  <span class="slider round"></span>
    ...
    <script type='text/javascript'>
     var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        // JavaScript code to perform an action on each object
       // alert("Processing object: {{ $orderitem->id }}");

      function deliver(id,productId,customer,qnt,reference,unitId,vehicle,driver,address,price,vat,invoice,vatperitem,totalwithVat , responseData,prize,discount,uniqueNumber,total,vatperitem,picngId){
...
if(invoice.checked){
  ...
  noinoive();
   ...
   //alert('wwwwwwwwwwwwwwwwwwwwwwwwwww'+ invoiceId);
       ...
       var unit = document.getElementById("unitId-{{ $orderitem->id }}").value
      ...
      // alert('discount value'+discountper);
       ...
       if(invoice.checked){
        ...
        deliver({{ $orderitem->id }},{{ $orderitem->productId}},{{ $orderitem->customerId}},qnt ,"{{ $orderitem->reference}}",unit ,vehicle,driver,address,price,vat,invoice,vatperitem,totalwithVat,customer,prize,discount,uniqueNumber,total,vatperitem);
...
var totalwithVat = document.getElementById("Totalvalincl").value
 ...
 var discountTotal = document.getElementById("discountTotal").value
 var discount = document.getElementById("discount-{{ $orderitem->id }}").value
 var unit = document.getElementById("unit-{{ $orderitem->unitId }}").value
...
// alert('discount value'+discountper);
 ...
 if(invoice.checked){
  ...
  deliver({{ $orderitem->id }},{{ $orderitem->productId}},{{ $orderitem->customerId}},qnt ,"{{ $orderitem->reference}}",unit,vehicle,driver,address,price,vat,invoice,vatperitem,totalwithVat,customer,prize,discount,uniqueNumber,total,vatperitem,picngId);
...
@foreach ($orderitems as $orderitem)
    @php $unit   = App\Models\Porduct::where('id',$orderitem->productId)->value('unitPackId'); @endphp
    @php $tmpUnit = $unittypes[$unit]; @endphp
    @php $tmpProduct = $porducts[$orderitem->productId]; @endphp
    @php $qnt   = App\Models\Pickingslip::where('orderitemId',$orderitem->id)->where('stateId','<>',45)->sum('qnt'); @endphp
    @php $Id    = App\Models\Pickingslip::where('orderitemId',$orderitem->id)->value('id'); @endphp
    @php $Ids[] = $Id; @endphp
    <tr>
        ...
        <td class="centered" width="150px"> <input type="input" class="form-control" class="large" onchange="vat({{ $orderitem->id }})" id="quantity-{{ $orderitem->id }}" value="{{ $qnt }}"  name="quantity" onclick="noedit()" readonly ></td>
    ...
    <input type="hidden" id="unit-{{$orderitem->unitId}}" name="unit"  value="{{$orderitem->unitId}}">
    <input type="hidden" id="quantity-{{ $orderitem->id}}" value="{{$qnt}}" name="quantity" >
...
<tr width="200px">
    ...
    <td class="highlights"></td>
    <td class="highlights"></td>
    <td class="highlights" class="centered" ><input type="button" onclick="deliverytype() , changestate();" class="btn btn-dark" id="vehicle1" name="vehicle1" value="Generate Delivery Note" ></td>
...
<script>
    ...
    document.getElementById("check-"+orderItemId).checked = true;
...
<input type="hidden" id="pickslip" value="{{ implode(',', $Ids) }}" name="Id" >
...
<script type='text/javascript'>
    ...
    function changestate(){
        ...
        var pickigslips = document.getElementById("pickslip").value ;

       // alert('over'+pickigslips);

        

$.ajax({
url: "{{ route('changestatepickingslip') }}",
type: 'post',
data: {_token: CSRF_TOKEN, pickigslips :pickigslips },
dataType: 'json',
success: function(response){
