...
<title>View Users Form - Laravel 8 CRUD</title>
...
<a class="btn btn-outline-info" href="{{ route('userdetails.index') }}" enctype="multipart/form-data"> Back</a>
</div>
</div>
</div>
@if(session('status'))
...
<input type="text" name="userPosition" class="form-control form-control- sm" value="{{$userdetail->userPosition}}"  placeholder="User Position">
@error('userPosition')
<div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
@enderror
</div>
