...
@endif
<form action="{{ route('invoice_items.store') }}" method="POST" enctype="multipart/form-data"onsubmit="preventFormSubmission(event)">
@csrf
...
<input   name="invoicesId"  class="form-control form-control-sm" value="{{$invoicesId}}" hidden>
@error('ordersId')
<div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
@enderror
</div>
