...
<div>
<form action="{{ route('workspaces.index', ['prntReport' => 'PRODUCTION_LIST'])}}" method="GET"  >
        ...
        @php $tmpMachinetype = $machinetypes[$machineryId ]; @endphp
            ...
            foreach ($machine['productions'] as $production) {
                foreach ($production['items'] as $item) {
                    ...
                    $tmpProcess = $processtypes[$production['details']['processId']] ?? null ;
                }
            }
            ?>
            <div class="mb-3">
                ...
                <tr>
                <th style="width: 900px;" > {{ $tmpMachinetype->name }}  <h3>

                </h3></th>
                

                @if ( $tmpProcess->name == 'Bagging')

                <th style="text-align: right;" >

                {{ $totalBales }}bales &nbsp;&nbsp;&nbsp; {{ $totalQuantity }}kgs
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @php $tmpProcesstype = $processtypes[$production['details']['processId']] ?? null ; @endphp
                            <tr>
                            <td class="no-print">
                            {{ \Carbon\Carbon::parse($production['details']['created_at'])->format('d D H:i') }}
                                ...
                                <td>
                                    <ul> <?php $totalItemsPerProduction = 0; ?>
                                        ...
                                        <span style="margin-right: 30px;" class="no-print"> {{ $item['created_at'] }}</span>
                                        ...
                                        @else
                                        <span style="margin-right: 30px;" class="no-print"> {{ $item['created_at'] }}</span>
                                        <span style="margin-right: 30px;">    {{ $item['quantity'] }} Bale</span>
                                        ...
                                        <span style="margin-right: 10px;"><strong>{{ $item['weight'] }} </strong></span>
                ...
                @endphp
                                            ...
                                            @foreach($groupedProducts as $productName => $productData)
                                                <li>
                                                {{ $productName }}<strong> --- {{ $productData['quantity'] }}</strong>-- {{ $productData['unitType']->name }}
                                                    <ul>
                                                        @foreach($productData['items'] as $item)
                                                            <!-- <li>
                                                                <span class="no-print" style="margin-right: 30px;">{{ $item['created_at'] }}</span>
                                                                @if($tmpProcesstype->name != 'Extruding')
                                                                    <span style="margin-right: 30px;">{{ $item['quantity'] }} Bale</span>
                                                                @endif
                                                                <span class="no-print" style="margin-right: 30px;">Job: {{ $item['jobcarditemId'] }}</span>
                                                                @if($item['weight'] != null && $item['weight'] != '0.00')
                                                                    <span style="margin-right: 10px;"><strong>{{ $item['weight'] }}</strong></span>
                                                                @endif
                                                            </li> -->
                                ...
                                <ul>
                                    <li>  <span style="margin-right: 30px;" ><strong></strong></span><strong> {{ $totalItemsPerProduction }} </strong></li>
                                    </ul>
                                </td>
                            </tr>
                        @endforeach
