...
<head>
  ...
  <script src="{{ asset('public/build/assets/chemicalorder.js') }}" type="module" ></script>  -->

       @viteReactRefresh
        @vite(['resources/css/app.css', 'resources/js/chemicalorder.tsx'])
...
<script>
    ...
    window.chemicalTypesData = [
        @foreach($chemicalTypes as $chemicalType)
            {
                id:   {{ $chemicalType->id }},
                name: "{{ addslashes($chemicalType->name) }}"
            }{{ !$loop->last ? ',' : '' }}
        @endforeach
    ];
