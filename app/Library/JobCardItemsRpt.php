...
class JobCardItemsRpt extends Fpdf
{
...
static function uniqidRealVal($prefix = 11, $lenght = 11)
{
...
public static function jobcard(Request $request) {
    ...
    // $jobcarditems=DB::table('jobcarditems')->where('JobcardId', $JobcardId)->get();
  
    // View::share('jobcard', $jobcard);
    //View::share('jobcardLists',$jobcarditems);
    
    
    $productId = $request->get('productId');
   // $jobcarditemList = DB::table('jobcarditems')->where('JobcardId', $JobcardId)->get(['id','jobCardId','name','productId','processId','qnt','unitId']);
    ...
    $pdf->Cell(276,12,'Job Card',1,0,'C');
    $pdf->Ln();
    $pdf->SetFont('Arial','',7);
    $id = ''.$JobcardId;
    if ($jobcard <> null) {
    ...
    $tmpProductType = $productKeys[$jobcard->productId];
    $product = ''.$tmpProductType->name;
    $tmpUnitType = $typesKeys[$jobcard->unitId]; 
    $unitId = ''.$tmpUnitType->name;
    $tmpMaterialType = $typesKeys[$porduct->materialTypeId];
    $vall = ''.$tmpMaterialType->name;
    $material  = ''.$vall;
    $tmpColorType = $typesKeys[$porduct->color];
    $valz = ''.$tmpColorType->name;
    $color = ''.$valz;
    $thickness = ''.$porduct->thickness;
    $width = ''.$porduct->product_Width;
    $lenght = ''.$porduct->product_length; 
    //$bagtype = ''.$jobcard->productId;
    $tmpbagType = $typesKeys[$jobcard->bagType]; 
    ...
    $pdf->Cell(15.66,4,'ID : ',1);

    $pdf->Cell(55.66,4,'Product',1);

    $pdf->Cell(30.66,4,'Unit',1);

    $pdf->Cell(30.66,4,'Material',1);

    $pdf->Cell(30.66,4,'Colour',1);

    $pdf->Cell(25.66,4,'Weidth(mm)',1);

    $pdf->Cell(25.66,4,'Length(mm)',1);

    $pdf->Cell(30.66,4,'thickness(mic)',1);

    $pdf->Cell(30.66,4,'Bag Type',1);

    $pdf->Ln();
    
    $pdf->Cell(15.66,5,$id,1);
    
    $pdf->Cell(55.66,5,$product,1);

    $pdf->Cell(30.66,5, $unitId,1,);

    $pdf->Cell(30.66,5,$material,1,);

    $pdf->Cell(30.66,5,$color,1,);

    $pdf->Cell(25.66,5,$width,1,);

    $pdf->Cell(25.66,5,$lenght,1,);

    $pdf->Cell(30.66,5,$thickness,1,);

    $pdf->Cell(30.66,5,$bagtype,1,);
    $pdf->Ln();
   ...
   // $barcodeVal = JobCardItemsRpt::uniqidRealVal();
    ...
    $pdf->Cell(276,12,'Job Card items',1,0,'C');
    $pdf->SetFont('Arial','',7.5);
    $pdf->Ln();
    //$id = ''.$jobcard;
    $headerPackingProduct = array('Id','Name','Product','Process','Quantity','Type');
    $packagingProductPersist = new JobCardItemsRpt();
    //$dataPP = $packagingProductPersist;
    //$packageArray = $packagingProductPersist->fpdfObjectsByProductId($productId);
    // echo "<pre>";
    // print_r($packageList);
    $jobcardArray = array ();
    $sizesList = array(46, 46, 46, 46, 46, 46, 46,46);
    $pdf->BasicTableWithSizes($headerPackingProduct, $jobcardArray ,$sizesList);
    $jobcarditemList = DB::table('jobcarditems')->where('JobcardId', $JobcardId)->get(['id','name','productId','processId','qnt','unitId']);
    $pdf->SetFont('Arial','',6.5);
    // echo "<pre>";
    // print_r($jobcardList);
    // exit;
    foreach($jobcarditemList as $jobcard)
	{
        $i=0;
        foreach($jobcard as $val)
         {  if ($i==2)  
            ...
            if ($i==3)  
            {
              $processType = $typesKeys [$val]; 
              $val = ''.$processType->name;
            }
            if ($i==5)  
            {
