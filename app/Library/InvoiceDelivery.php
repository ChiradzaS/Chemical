...
class InvoiceDelivery extends Fpdf
{
    ...
    function Footer() {
        ...
        // Set Y position to 15 mm from the bottom
        $this->SetY(-20);

        // Set X position to 10 mm from the left
        $this->SetX(10);

        // Set font size and color for the signature
        $this->SetFont('Arial', '', 10); // Set font to Arial, regular, size 10
        $this->SetTextColor(0, 0, 0); // Set text color to black

        // Add signature content
        $this->Cell(0, 6, '_____________________________________', 0, 1, 'L');
        $this->Cell(0, 4, 'Signature', 0, 0, 'L');
    }
...
function Content($custval) {
    // Set cell background color to light gray
    $this->SetFillColor(211, 211, 211);

    // Add a cell with content
    $this->Cell(192, 12, 'Delivery Note To: ' . $custval, 1, 0, 'C', true); // The last parameter (true) is used to fill the cell with the specified color
}
...
public static function delivery(Request $request) { 
   ...
   // $specificValue = $jsonData['value'];
    $dataValue = $request->query('data');

    $originalString = $dataValue ;
    ...
    $drivertypes = DB::table('employees')
    ->get();

    $driverKeys = array();
    foreach (  $drivertypes as $drivertype) {
        $driverKeys[ $drivertype->id] =  $drivertype;
    }

    $vehicletypes = DB::table('types')
                       ->get();

    $vehicleKeys = array();
    foreach (  $vehicletypes as $vehicletype) {
        $vehicleKeys[ $vehicletype->id] =  $vehicletype;
    }
    ...
    ->where('uniqu',$cleanedString  )
    ->select('productId', 'unitId', 'qnt','customerId')
    //->distinct()
    ->get();

    $customerInfo = DB::table('deliveries')
    ->where('uniqu',$cleanedString  )
    ->first();

   /// dd('customer info from deliveries'.$cleanedString  );
    ...
    $total = DB::table('invoice_items')->where('invoicesId', $customerInvoice)
...
$totalVat = DB::table('invoice_items')->where('invoicesId',$customerInvoice)
...
$totalexclVAT =  $total - $totalVat;
   ...
   //$customer = $deliverinfo->customerId;
    ...
    $pdf = new InvoiceDelivery();
    $title = 'Invoice & Delivery ';
    $pdf->SetTitle($title);
   // $pdf->Image('C:\xampp\htdocs\logo', 10, 10, 50, 0, 'PNG');
 ...
 $tmpY = $pdf->getY();
   ...
   // $pdf->Ln();
    ...
    $pdf->Cell(40, 5, 'SAILING PACKAING', 0, 0, 'L', false);
    $pdf->Ln();
    $pdf->SetFont('Arial','',7);
    ...
    $itemsLists = DB::table('invoice_items')->where('invoicesId',$customerInvoice)->get();
    // echo "<pre>";
    ...
    foreach($itemsLists as $itemsList){
        ...
        $pdf->Cell(20,7, '', 0, 0, 'L', false);
        $pdf->Cell(20,7, ''.$itemsList->price, 0, 0, 'L', false);
        $pdf->Cell(30,7, '', 0, 0, 'L', false);
...
// Add the text with a different color for one letter
$pdf->Cell(13, 5, 'SA', 0, 0, 'L', false);
$pdf->SetTextColor(0, 0, 0); // Set text color to red
$pdf->Cell(4, 5, 'I', 0, 0, 'L', false);
$pdf->SetTextColor(0, 0, 0); // Set text color back to black
$pdf->Cell(22, 5, 'LING', 0, 0, 'L', false); // Print 'LING' in black
$pdf->SetTextColor(150, 150, 150); // Set text color to red for the '&'
$pdf->SetFont('Arial','',36);
$pdf->Cell(8, 5, ' ', 0, 0, 'L', false);
$pdf->SetFont('Arial','',27);
$pdf->SetTextColor(0, 0, 0); // Set text color back to black
$pdf->Cell(12, 5, 'PACKAGING', 0, 0, 'L', false); // Print 'PACKAGING' in black
    ...
    $driver = $driverKeys[$customerInfo->driver];
    $driverval = $driver->name;

    $vehicle = $vehicleKeys[$customerInfo->vehicleReg];
    $vehicleval = $vehicle->name;

    
    
    $headerPackingProduct = array(' Date : '.$formattedDate,'Vehicle : '.$vehicleval,'Driver : '.$driverval);
    $packagingProductPersist = new InvoiceRpt();
    //$dataPP = $packagingProductPersist;
    //$packageArray = $packagingProductPersist->fpdfObjectsByProductId($productId);
    // echo "<pre>";
    ...
    $pdf->Cell(110,10, 'Description', 0, 0, 'L', false);
    $pdf->Cell(50,10, 'Package', 0, 0, 'L', false);
    $pdf->Cell(50,10, 'Quantity', 0, 0, 'L', false);
    $pdf->Ln();
    ...
    foreach( $deliveries as  $deliverie){
        ...
        $productId = $productKeys[ $deliverie->productId];
        $val = ''.$productId->name;

        

        $unitId = $unitKeys[ $deliverie->unitId];
        $vat = ''.$unitId->name;

     

        $pdf->Cell(110,7, ''. $val , 0, 0, 'L', false);
        
        $pdf->Cell(50,7, ''.$vat, 0, 0, 'L', false);
        $pdf->Cell(50,7, ''.$deliverie->qnt, 0, 0, 'L', false);
